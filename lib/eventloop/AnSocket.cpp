...
ssize_t AnSocket::Read(void* pstBuffer, size_t dwLength)
{
    if(m_bIsServer && m_iOwnerFd != -1)
    {
        return -1;
    ...
    return ::read(m_iOwnerFd, pstBuffer, dwLength);
}
...
ssize_t AnSocket::Write(void* pstBuffer, size_t dwLength)
{
    if(m_bIsServer && m_iOwnerFd != -1)
    {
        return -1;
    }
    return ::write(m_iOwnerFd, pstBuffer, dwLength);
}



ssize_t AnSocket::Recv(int fd, void* pstBuffer, size_t dwLength)
{
    return ::recv(fd,  pstBuffer, dwLength, 0);
}

ssize_t AnSocket::Read(int fd, void* pstBuffer, size_t dwLength)
{
...
ssize_t AnSocket::Send(int fd, void* pstBuffer, size_t dwLength)
{
    return ::send(fd,  pstBuffer, dwLength, 0);
}

ssize_t AnSocket::Write(int fd, void* pstBuffer, size_t dwLength)
{
