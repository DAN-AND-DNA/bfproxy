...
EventLoop::EventLoop() noexcept:
    ...
    m_stActiveEvents(16)
{
    if(this->m_iEpollFd == -1)
    {
        dan::log::BFLog::FATAL("epoll create failed:{}", ::strerror(errno));
        ::exit(-1);
    }
    dan::log::BFLog::INFO("epoll create success, epollfd:{}", this->m_iEpollFd);
...
EventLoop::~EventLoop()
{
    ...
    dan::log::BFLog::INFO("epoll close success");
}
...
void EventLoop::Fire(int iReady) noexcept
{
   ...
   // printf("get events\n");
	...
	//FIXME unix.EPOLLIN = unix.EPOLLIN | unix.EPOLLERR | unix.EPOLLHUP
        // read
