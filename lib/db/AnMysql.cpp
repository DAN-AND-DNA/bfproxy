...
{
    dan::danMysqlDo stDo(m_pstRawConn.get(), "insert into bf_user_0(userid,openid,updatetime,player) value(?,?,unix_timestamp(),?)  on duplicate key update player = ?, updatetime = unix_timestamp()");
